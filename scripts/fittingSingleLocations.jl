...
for (ii, CW) = enumerate(waveletExamples)
    ...
    fittingSinglePath(N, tuple(), reprFun=(identity, identity), extraName="SPACE", cw=CW, namedCW=shortNames[ii], totalMins=totalMins)
end
...
for (ii, CW) = enumerate(waveletExamples)
    ...
    @sync @distributed for (w2, w1) in listOfLocations
        ...
        fittingSinglePath(N, (w2, w1,), reprFun=(identity, identity), extraName="SPACE", cw=CW, namedCW=shortNames[ii], totalMins=totalMins)
    end
end
