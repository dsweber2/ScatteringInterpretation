...
function pullAndTrain(setProbW, objSPC; normDiscount=.01, N=1000)
    ...
    objSPC(init)
    pathOfDescent, err = ScatteringTransformExperiments.fitReverseSt(N, init, opt=opt, obj=objSPC, ifGpu=identity, adjust=true, relGradNorm=1e-18);
    pop[:,trainI] = dct(pathOfDescent[:, 1, 1, end])
    ScatteringTransformExperiments.adjustPopulation!(setProbW, pop)
